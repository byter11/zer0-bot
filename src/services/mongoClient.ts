...
export default class MongoDatabase implements Database {
  ...
  async setLastMatch(id: string, matchId: string) {
    return this.collections.users?.updateOne(
      { discordId: id },
      { $set: { "valorant.lastMatch": matchId } }
    );
  }

  async webhook(guildId: string) {
    return this.collections.webhooks
      ?.findOne({ guildId: guildId })
      .then((webhook) => webhook as Webhook);
  }
  ...
  async webhooks() {
    return this.collections.webhooks
      ?.find()
      .toArray()
      .then((webhooks) => webhooks as Webhook[]);
  }

  async registerWebhook(webhook: Webhook) {
    return this.collections.webhooks?.updateOne(
      { guildId: webhook.guildId },
      { $set: { id: webhook.id, url: webhook.url } },
      { upsert: true }
    );
  }
}
